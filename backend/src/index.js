...
async function extractWithOpenAI(url, pageContent) {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You extract structured recipes from web page content. Return valid JSON matching this schema: ${RECIPE_SCHEMA}. Estimate nutrition per serving if not provided, but PREFER any nutrition numbers stated in the text (e.g. "B:" protein, "T:" fat, "W:" carbs in Polish). Keep ingredient and step text in the SAME language as the source. Use realistic aisle categories. The "caption" field, when present, is the author's full post description and is the most reliable source of ingredients and steps — extract from it carefully. IMPORTANT — ingredient sections: captions often split ingredients into sub-sections with their own headings (e.g. "For the chicken:", "Marinade:", "Sauce 👇", "Topping", "To serve"). Detect these headings (a line ending with ":", starting with "For the", or an emoji/label before a sub-list) and set each ingredient's "group" to that heading text. If a recipe uses ANY sections, assign EVERY ingredient to a group — give leftover/base items a sensible label like "Main", "Base" or "To serve" rather than leaving them blank. If the recipe has NO sub-sections at all, leave "group" as an empty string for every ingredient. Only invent a recipe if no real content is available. For "tags": pick EXACTLY ONE best-fit category from [Quick, Dinner, Breakfast, Lunch, Vegetarian, High-protein]. Quick = under 20 min. Only return one tag.`,
      },
      {
        role: 'user',
